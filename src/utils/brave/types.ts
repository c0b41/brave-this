...
export interface NewsSearchOptions
  extends Pick<BraveSearchOptions, 'country' | 'search_lang' | 'ui_lang' | 'count' | 'offset' | 'spellcheck' | 'safesearch' | 'freshness' | 'extra_snippets'> {}
...
// Define the structure for the Provider
interface Provider {
  type: 'internal' // Can be extended with other literal types if needed
  name: string
  url: string
}

// Define the structure for ChatLLM
export interface ChatLLM {
  ...
  trigger: boolean
  possible: boolean
  context: any[] // Consider defining a more specific type if 'context' has a consistent structure
  ...
  bo_callback_follow_up: string
  bo_callback_close_chat: string
  bo_callback_open_leo: string
}

// Define the structure for UnitConversionData
...
export interface Timeseries {
  time_range: string
  ts_exchange_rate: [string, number][]
}
...
export interface Translator {
  generic: boolean
  from_language: LanguageInfo
  from_language_detected: LanguageInfo
  from_placeholder: string
  to_language: LanguageInfo
...
export interface ConvertedTime {
  generic_name_abbreviation: string
  timezone_display_abbreviation: string
...
export interface DstRule {
  dst_adjust_1: string
  dst_adjust_2: string
  dst_end_date_rule: string
  dst_end_month: string
  dst_end_time: string
  dst_start_date_rule: string
  dst_start_month: string
...
export interface TimezoneDetails {
  ...
  utc_offset: string
  utc_offset_minutes: number
  iana_timezone_id: string
  windows_timezone_id: string
  observing_dst: boolean
  dst_rule: DstRule
}
...
export interface RichResult {
  ...
  calculator: null // Or a more specific type
  chatllm: ChatLLM | null
  currency: Currency // Or a more specific type
  usElection: null // Or a more specific type
  news: null // Or a more specific type
  unitConversion: UnitConversionData | null
  timer: null // Or a more specific type
  stopwatch: boolean
...
// Define the main RichData structure
